...
public:
	...
	// start playback (one time through)
	inline void start() {
		if (events.size()) {
			currentPlay = 0.0f;
	...
	// start playback (in continuous loop)
	inline void loop() {
		if (events.size()) {
			currentPlay = 0.0f;
			playEnd = static_cast<float>(midi->getFileDurationInSeconds());
	...
	inline void process(std::function<void(bool, int, int)> callback) {
		if (playingFlag) {
	...
	inline bool operator==(OtMidiFile& rhs) {
