...
class OtAlignCircuitsTask : public OtEditorTask {
public:
	// alignment types
	...
	// constructor
	inline OtAlignCircuitsTask(OtAudio* s, Alignment a) : audio(s), alignment(a) {}
	...
	inline void perform() override {
		for (auto id : audio->getSelected()) {
			...
			locations.emplace_back(id, ImVec2(circuit->x, circuit->y), ImVec2(circuit->w, circuit->h));
		}
