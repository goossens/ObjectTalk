...
def printTable(table, type, name, filter, format):
	result = '''
...
if __name__ == "__main__":
	...
	with open("OtUnicode.h", "w") as tables:
		...
		tables.write(printTable(letters, "Range16", "letters16", filter16, formatStride))
		tables.write(printTable(letters , "Range32", "letters32", filter32, formatStride))

		lower = calculateStrides(buildRangeTable("UnicodeData.txt", lambda line : line[2] == "Ll"))
		tables.write(printTable(lower, "Range16", "lower16", filter16, formatStride))
		tables.write(printTable(lower , "Range32", "lower32", filter32, formatStride))

		upper = calculateStrides(buildRangeTable("UnicodeData.txt", lambda line : line[2] == "Lu"))
		tables.write(printTable(upper, "Range16", "upper16", filter16, formatStride))
		tables.write(printTable(upper, "Range32", "upper32", filter32, formatStride))

		numbers = calculateStrides(buildRangeTable("UnicodeData.txt", lambda line : line[2] == "Nd"))
		tables.write(printTable(numbers, "Range16", "numbers16", filter16, formatStride))
		tables.write(printTable(numbers, "Range32", "numbers32", filter32, formatStride))

		whitespace =calculateStrides( buildRangeTable("PropList.txt", lambda line : line[1] == "White_Space"))
		tables.write(printTable(whitespace, "Range16", "whitespace16", filter16, formatStride))

		xidStart = calculateStrides(buildRangeTable("DerivedCoreProperties.txt", lambda line : line[1] == "XID_Start"))
		tables.write(printTable(xidStart, "Range16", "xidStart16", filter16, formatStride))
		tables.write(printTable(xidStart, "Range32", "xidStart32", filter32, formatStride))

		xidContinue = calculateStrides(buildRangeTable("DerivedCoreProperties.txt", lambda line : line[1] == "XID_Continue"))
		tables.write(printTable(xidContinue, "Range16", "xidContinue16", filter16, formatStride))
		tables.write(printTable(xidContinue, "Range32", "xidContinue32", filter32, formatStride))

		caseRanges = compressUpperLower(compressCaseRanges(createCaseRanges("UnicodeData.txt")))
		tables.write(printTable(caseRanges, "CaseRange16", "case16", filter16, formatCase))
		tables.write(printTable(caseRanges, "CaseRange32", "case32", filter32, formatCase))
