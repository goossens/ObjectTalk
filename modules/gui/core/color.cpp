...
struct OtColor {
	OtColor() {}
	OtColor(int _r, int _g, int _b, int _a) : r(_r), g(_g), b(_b), a(_a) {}
	int r = 0, g = 0, b = 0, a = 0;
};
...
static OtColor toColor(std::vector<double> parts, const std::string& text) {
	// sanity check
	for (const auto& part : parts) {
		if (part < 0 || part > 255) {
			OtExcept("Invalid color specification[%s]", text.c_str());
		}
	}
...
static std::vector<double> parseParts(size_t count, const std::string& text, double base) {
	// create copy of string
	std::string str = text.substr(count);
	...
	for (const auto& part : parts) {
		if (part.back() == '%') {
			result.push_back(std::stod(part) * base / 100.0);
...
static OtColor parse(const std::string& text) {
	// create copy of string
	std::string str = text;
	...
	if (str.length() && str.front() == '#') {
		if (str.length() == 4) {
			int64_t iv = std::stoi(str.substr(1), 0, 16);
		...
		std::vector<double> parts = parseParts(3, str, 255);
		parts.push_back(255);
		return toColor(parts, text);

	} else if (str.rfind("rgba(", 0) == 0) {
		std::vector<double> parts = parseParts(4, str, 255);
		return toColor(parts, text);
	...
	// I don't know what color that was but it looks transparent now
	OtExcept("Invalid color specification[%s]", text.c_str());
	return OtColor();
}
...
glm::vec4 OtColorParseToVec4(const std::string& css) {
	OtColor color = parse(css);
	return glm::vec4(color.r / 255.0, color.g / 255.0, color.b / 255.0, color.a / 255.0);
}
