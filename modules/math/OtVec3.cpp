...
OtObject OtVec3Class::add(OtObject object) {
	if (object.isKindOf<OtVec3Class>()) {
		...
		OtError("Expected a [Vec3], [Real] or [Integer] as the operand, not a [{}]", object->getType()->getName());
		return nullptr;
	}
}
...
float OtVec3Class::distance(OtObject object) {
	if (object.isKindOf<OtVec3Class>()) {
		...
		OtError("Expected a [Vec3] as the operand, not a [{}]", object->getType()->getName());
		return 0.0f;
	}
