...
void OtVec4Class::init(size_t count, OtObject* parameters) {
	if (count == 4) {
		...
		OtExcept("[Vec4] constructor expects 0, 1 or 4 arguments (not %ld)", count);
	}
}
...
OtObject OtVec4Class::add(OtObject object) {
	if (object.isKindOf<OtVec4Class>()) {
		...
		OtExcept("Expected a [Vec4], [Real] or [Integer] as the operand, not a [%s]", object->getType()->getName().c_str());
		return nullptr;
	}
}
...
OtObject OtVec4Class::lerp(OtObject object, float delta) {
	if (object.isKindOf<OtVec4Class>()) {
		...
		OtExcept("Expected a [Vec4] as the operand, not a [%s]", object->getType()->getName().c_str());
		return nullptr;
	}
}
