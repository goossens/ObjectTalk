...
public:
	...
	std::string gethome()
	{
		...
		if (result == UV_ENOBUFS)
			OT_EXCEPT("fs.gethome of size %d does not fit in buffer of size %d", size, UV_ENOBUFS);

		else if (result)
			OT_EXCEPT("fs.gethome failed: %s", uv_strerror(result));
	...
	std::string gettmp()
	{
		...
		if (result == UV_ENOBUFS)
			OT_EXCEPT("fs.gettmp of size %d does not fit in buffer of size %d", size, UV_ENOBUFS);

		else if (result)
			OT_EXCEPT("fs.gettmp failed: %s", uv_strerror(result));
	...
	void chdir(const std::string& path)
	{
		...
		if (result)
			OT_EXCEPT("fs.chdir failed: %s", uv_strerror(result));
	}
	...
	std::string getcwd()
	{
		...
		if (result == UV_ENOBUFS)
			OT_EXCEPT("fs.getcwd of size %d does not fit in buffer of size %d", size, UV_ENOBUFS);

		else if (result)
			OT_EXCEPT("fs.getcwd failed: %s", uv_strerror(result));
	...
	OtObject ls(const std::string& path)
	{
		...
		{
			if (result)
				OT_EXCEPT("fs.ls failed: %s", uv_strerror(result));
	...
	size_t filesize(const std::string& path)
	{
		...
		if (result)
			OT_EXCEPT("fs.filesize failed: %s", uv_strerror(result));
	...
	void rm(const std::string& path)
	{
		...
		if (result)
			OT_EXCEPT("fs.rm failed: %s", uv_strerror(result));
	}
