...
public:
	...
	typedef enum {
		...
		methodOpcode,
		exitOpcode,
		pushInstructionOpcode,
		popInstructionOpcode,
		reserveOpcode,
		pushSymbolOpcode,
		popSymbolsOpcode,
		pushTryOpcode,
	...
	inline size_t pushInstruction(size_t start) { emitOpcode(pushInstructionOpcode); return emitInstruction(start); }
	inline void pushInstruction(size_t start, size_t end) { emitOpcode(pushInstructionOpcode); return emitInstruction(start, end); }
	inline void popInstruction() { emitOpcode(popInstructionOpcode); }
	...
	// patch instruction end
	inline void patchInstruction(size_t index, size_t end) { instructions[index].second = end; }
	...
	inline size_t getInstructionStart(size_t index) { return instructions[index].first; }
	inline size_t getInstructionEnd(size_t index) { return instructions[index].second; }
...
private:
	...
	inline size_t emitInstruction(size_t start) {
		auto index = instructions.size();
		instructions.emplace_back(std::make_pair(start, 0));
		emitNumber(index);
		return index;
	}

	inline void emitInstruction(size_t start, size_t end) {
		auto index = instructions.size();
		instructions.emplace_back(std::make_pair(start, end));
		emitNumber(index);
	}
