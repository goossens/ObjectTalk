...
public:
	...
	typedef enum {
		...
		pushStackObjectOpcode,
		pushStackMemberOpcode,
	...
	inline void pushStackObject(size_t slot) { emitOpcode(pushStackObjectOpcode); emitNumber(slot); }
	inline void pushStackMember(size_t slot, OtID member) { emitOpcode(pushStackMemberOpcode); emitNumber(slot); emitID(member); }
