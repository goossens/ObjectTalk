	...
	while (pc < end) {
		try {
			switch (bytecode->getOpcode(pc)) {
				...
				case OtByteCodeClass::pushInstructionOpcode:
					stack.pushInstruction(bytecode->getNumber(pc));
				...
				case OtByteCodeClass::popInstructionOpcode:
					stack.popInstruction();
					break;
			...
			if (tryCatch.size()) {
				...
				// find start and end of instruction
				size_t instruction = stack.getInstruction();
				size_t start = bytecode->getInstructionStart(instruction);
				size_t end = bytecode->getInstructionEnd(instruction);
