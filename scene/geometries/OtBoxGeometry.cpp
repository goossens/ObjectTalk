...
void OtBoxGeometryClass::init(size_t count, OtObject* parameters) {
	...
	if (count) {
		switch (count) {
			...
			default:
				OtError("Too many parameters [{}] for [BoxGeometry] constructor (max 3)", count);
		}
...
nlohmann::json OtBoxGeometryClass::serialize(std::string* basedir) {
...
void OtBoxGeometryClass::deserialize(nlohmann::json data, std::string* basedir) {
