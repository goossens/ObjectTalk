...
void OtCircleGeometryClass::init(size_t count, OtObject* parameters) {
	...
	if (count) {
		switch (count) {
			...
			default:
				OtError("Too many parameters [{}] for [CircleGeometry] constructor (max 3)", count);
		}
...
nlohmann::json OtCircleGeometryClass::serialize(std::string* basedir) {
...
void OtCircleGeometryClass::deserialize(nlohmann::json data, std::string* basedir) {
