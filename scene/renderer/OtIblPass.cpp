...
//
//	OtSceneRenderer::renderIblPass
//

void OtSceneRenderer::renderIblPass(OtSceneRendererContext& ctx) {
	// see if the cubemap has changed
	auto& ibl = ctx.scene->getComponent<OtIblComponent>(ctx.iblEntity);
	OtCubeMap& cubemap = ibl.cubemap->getCubeMap();
	...
	if (cubemap.getVersion() != iblSkyMapVersion) {
		...
		pass.runComputeProgram(program, brdfLutSize / threadCount, brdfLutSize / threadCount, 1);
